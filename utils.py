...
def get_clip_logits(images, clip_model, clip_weights, counter, count, alpha=1, mean_loss=None, alpha1=None):
    # with torch.no_grad():
    ...
    if image_features.size(0) > 1:
        ...
        # Compute pred2 and loss2
        # logits[n] = logits[n] * (-a * (1 + conf_past - conf_now) * prob[n]);
        # if current conf_now is lower, increase the strength of logits adjustment
        clip_logits = logits_adjustment(
            clip_logits,
            prob_map2.squeeze(0)[pred2].detach().cpu().item() - mean_loss[pred2].detach().cpu().item(),
            counter,
            count,
            alpha
        )
    ...
    # Reflection mechanism: check whether the two predictions are the same
    if (pred == pred2):
...
def get_clip_logits_2(image_features, clip_model, clip_weights, counter, count, alpha=1, mean_loss=None, alpha1=None):
    with torch.no_grad():
